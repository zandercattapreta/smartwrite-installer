...
for repo in $REPOS; do
    ...
    for branch in "main" "master"; do
        ...
        if echo "$MANIFEST" | jq -e . >/dev/null 2>&1; then
            echo "     (Found manifest in '$branch' branch)"
            break
        else
            MANIFEST=""
        fi
    ...
    if [ -n "$MANIFEST" ]; then
        ...
        echo "     (Skipping: No valid manifest.json found in main or master branches)"
    fi
done
...
check_and_build() {
    ...
    cd "$DIR"
    if ! npm install; then
        echo -e "    ${RED}✗ npm install failed. Check your internet connection or package.json.${NC}"
        cd - > /dev/null
        return 1
    fi
    ...
    if [ -f "package.json" ] && grep -q '"build"' "package.json"; then
        ...
        if ! npm run build; then
            echo -e "    ${RED}✗ npm run build failed. Check for TypeScript errors.${NC}"
            cd - > /dev/null
            return 1
        fi
    fi
